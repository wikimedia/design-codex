...
describe( 'matches the snapshot', () => {
	...
	test.each( cases )( 'Case %# %s: (%p) => HTML', ( _, props, slot ) => {
		const wrapper = shallowMount( CdxChip,
			{
				props: props,
				slots: { default: slot }
			} );
...
describe( 'Chip Icon', () => {
	...
	it( 'shows user provided icon if provided when status is notice', () => {
		const wrapper = shallowMount( CdxChip, { props: { icon: cdxIconCamera } } );
	...
	it( 'does not allow custom icon for non-notice status', () => {
		const wrapper = shallowMount( CdxChip, { props: { status: 'success', icon: cdxIconCamera } } );
