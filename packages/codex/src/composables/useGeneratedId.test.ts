...
describe( 'useGeneratedId', () => {
	describe( 'when called without an identifier argument', () => {
		describe( 'and parent component has no "id" specified', () => {
			const Component = {
			...
			it( 'increments the counter each time it is called and appends that value as a suffix', () => {
				...
				expect( getSuffix( wrapper2.vm.id ) ).toBeGreaterThan(
					getSuffix( wrapper1.vm.id ) );
			} );
		} );
	...
	describe( 'when called with an identifier argument', () => {
