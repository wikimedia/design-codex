export default function useStringHelpers(): {
	regExpEscape: ( value: string ) => string,
	splitStringAtMatch: ( query: string, title: string ) => [ string, string, string ]
	} {
	/**
	 ...
	 */
	function splitStringAtMatch( query: string, title: string ): [ string, string, string ] {
		if ( !query ) {
			return [ title, '', '' ];
		}
