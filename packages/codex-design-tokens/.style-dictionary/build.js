...
for ( const mode of modes ) {
	await ( await sdBase.extend( {
		// Use "source" for the mode tokens so that they override the base files in "include".
		source: [
			`src/modes/${ mode }.json`
		],
		...
		platforms: {
			stylesheet: {
				...
				files: [
					{
						destination: `theme-wikimedia-ui-mode-${ mode }.css`,
						format: 'css/variables',
						filter: isModeToken
					},
					{
						destination: `theme-wikimedia-ui-mixin-${ mode }.less`,
						format: 'css/variables',
						filter: isModeToken,
						options: {
							selector: `.cdx-mode-${ mode }()`
						}
					},
					// Full build of tokens including dark overrides.
					{
						destination: `theme-wikimedia-ui-mode-${ mode }.json`,
						format: 'json'
					}
				]
			}
